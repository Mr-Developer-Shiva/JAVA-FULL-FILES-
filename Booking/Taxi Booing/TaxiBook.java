...
public class TaxiBook extends Thread{
    ...
    TaxiBook(){
        for(int i = 1 ; i <= 4 ; i++){
            //Work with four taxies ( i = 1 , i = 4) , you can do 
            taxi t = new taxi(i);
            taxies.add(t);
        }
        /*
    ...
    static void bookTaxi(){     //get details for booking and allot taxi
        ...
        for(taxi t : taxies){
            if((t.isFree == true) && (t.currentPoint == pp)){       //assign a taxi when
            ...
            if(t.isFree == true ) {
                rej = 0 ;//means atleast one texi is free
            }
        }
        if(rej == 1 ){
        ...
        for(taxi t1 : taxies){      //get nearest taxies in another array list
            if(t1.isFree == true){
                if(Math.abs(t1.currentPoint - pp) == 1){
                    ...
                    nearestTaxies.add(t1);//just add free taxi if distance 
                }
            }
        } 
        if(nearestTaxies.size() == 1){  //if we have only one nearest taxi
            ...
            for(int i = 1 ; i < nearestTaxies.size();i++){
                ...
                if(min==Math.abs(nearestTaxies.get(i).currentPoint)){
                    iden++;
                }
            }
            if(iden == 0 ){
...
static void taxiDetails(){
    for(taxi t : taxies){
        ...
        System.out.print("\t\t Total Earning " + t.totalEarn);
        System.out.println("  ");
        System.out.println("Booking    CustomerID    From    To    PickupTime    DropTime    Amount");
